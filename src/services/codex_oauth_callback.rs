//! Single-shot local HTTP server that captures the OAuth `/auth/callback`
//! redirect for the Codex ChatGPT flow.
//!
//! - Binds `127.0.0.1:1455` (fixed — the port is part of the OAuth redirect
//!   URI registered with OpenAI).
//! - Reads the first request line, pulls `code` + `state` from the query
//!   string, returns a small success HTML to the browser.
//! - Resolves on first callback or on timeout; rejects mismatched `state`.
...
use crate::services::codex_oauth::CALLBACK_PORT;

/// Port-1455-in-use failure. Distinct so callers can fall back to the
...
impl std::fmt::Display for PortUnavailable {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "127.0.0.1:{CALLBACK_PORT} is in use; falling back to manual paste"
        )
    }
}
...
/// Binds the callback server and waits for one valid `/auth/callback` hit.
///
/// On success returns `Ok(CallbackOutcome { code })`. On a state mismatch
/// the browser sees a 400 and the call returns `Err`. On timeout returns
/// `Err`.
///
/// The caller is responsible for printing the "open this URL" hint — this
/// function only runs the socket side.
pub async fn wait_for_callback(expected_state: &str, timeout: Duration) -> Result<CallbackOutcome> {
...
async fn accept_one(listener: TcpListener, expected_state: &str) -> Result<CallbackOutcome> {
    // Loop until we see a real `/auth/callback` hit. Browsers probe `/`,
    // `/favicon.ico`, etc.; we answer those with 404 and keep waiting.
    loop {
        ...
        let request_line = match read_request_line(&mut stream).await {
            Ok(line) => line,
            Err(_) => {
                // Malformed request — close and keep listening.
...
/// Reads up to the first CRLF (or plain LF as a fallback). Bounded to
/// 8 KiB so a misbehaving client can't eat memory.
